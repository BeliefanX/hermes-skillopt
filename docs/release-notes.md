...
- Documents latest P0-P3 readiness/authoring additions:
  ...
  - `scout --digest`, `doctor --digest`, `eval-pack-inventory --digest`, and `eval-pack-doctor --digest` are the cron-safe default diagnostic surfaces; existing-run `review --digest` is digest-only/manual and not a scheduled default.
...
- Documents native-boundary/eval-maturity implementation updates:
  ...
  - Adopt requires strict gates, complete frozen runtime evidence, reviewer gate, native conflict guard, current SHA/proposed SHA/provenance hashes, and hashed-artifact crosschecks; cron-safe defaults are narrowed to digest-style scout/doctor/inventory/eval-pack-doctor monitoring, while review digest stays manual.
