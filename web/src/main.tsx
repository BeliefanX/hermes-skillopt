...
const dict = {
  en: {
    ...
    intent: 'Intent', smoke: 'Smoke', reviewOnly: 'Review-only', production: 'Production', smokeHelp: 'Fast smoke test; mock fallback is allowed and evidence is review-only.', reviewHelp: 'Staged review run for exploration; may use non-production evidence.', productionHelp: 'Requires explicit eval file, strict gate, and no mock behavior. Still staged-only with no auto-adopt.', decisionFirst: 'Decision-first review', blockers: 'Blockers', prodGate: 'Production gate', testGate: 'Test gate', evidence: 'Evidence class', artifacts: 'Artifacts', nextSafe: 'Next safe action', rawReview: 'Raw review JSON', expandArtifacts: 'Raw JSON/diff/report are expandable below.',
    evalPackTitle: 'Eval-pack one-click UX', evalPackSafety: 'Review-only by default. These actions never adopt a live skill; production promotion is intentionally not exposed here.', diagnoseEval: 'Diagnose eval coverage', generateDraft: 'Generate review draft', promoteDraft: 'Promote draft to curated review pack', draftPath: 'Draft input path', curatedOutput: 'Curated output path (optional)', overwrite: 'Overwrite existing pack', rawEvalPack: 'Raw eval-pack result',
  },
  zh: {
    ...
    intent: '意图', smoke: '冒烟', reviewOnly: '仅审查', production: '生产', smokeHelp: '快速冒烟；允许 mock fallback，证据仅用于审查。', reviewHelp: '用于探索的暂存审查运行；可能使用非生产证据。', productionHelp: '需要显式评测文件、strict 门禁且不允许 mock。仍然只写暂存，不会自动采用。', decisionFirst: '决策优先审查', blockers: '阻塞项', prodGate: '生产门禁', testGate: '测试门禁', evidence: '证据类别', artifacts: '产物', nextSafe: '下一步安全操作', rawReview: '原始审查 JSON', expandArtifacts: '原始 JSON/diff/report 可在下方展开。',
    evalPackTitle: '评测包一键流程', evalPackSafety: '默认仅审查。这些操作永不采用 live skill；此处有意不暴露生产 promotion。', diagnoseEval: '诊断评测覆盖', generateDraft: '生成审查草稿', promoteDraft: '提升为 curated review pack', draftPath: '草稿输入路径', curatedOutput: 'Curated 输出路径（可选）', overwrite: '覆盖已有评测包', rawEvalPack: '原始评测包结果',
  },
...
function App() {
  ...
  const evalPackAction = async (kind: 'doctor' | 'autopilot' | 'promote') => { setBusy(true); setMessage(''); try { const skill = evalPackSkill || runForm.skill; let out: EvalPackResponse; if (kind === 'doctor') { const qs = new URLSearchParams(); if (skill) qs.set('skill', skill); if (home) qs.set('home', home); out = await api<EvalPackResponse>(`/api/eval-pack/doctor?${qs.toString()}`); } else if (kind === 'autopilot') { out = await api<EvalPackResponse>('/api/eval-pack/autopilot', { method: 'POST', body: JSON.stringify({ skill, home, write_draft: true, overwrite: evalPackOverwrite }) }); const draft = out.draft; const path = typeof draft?.output_path === 'string' ? draft.output_path : typeof out.output_path === 'string' ? out.output_path : ''; if (path) setEvalPackDraftPath(path); } else { out = await api<EvalPackResponse>('/api/eval-pack/promote', { method: 'POST', body: JSON.stringify({ skill, home, input_path: evalPackDraftPath, output: evalPackOutput, overwrite: evalPackOverwrite, production: false }) }); } setEvalPackResult(out); setMessage(`${kind === 'doctor' ? t.diagnoseEval : kind === 'autopilot' ? t.generateDraft : t.promoteDraft} complete · review-only · no skill was adopted.`); } catch (e: unknown) { setMessage(errorMessage(e)); } finally { setBusy(false); } };
