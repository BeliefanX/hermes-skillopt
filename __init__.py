...
BOUNDARY_NOTE = "SkillOpt does not replace the Hermes curator: curator owns lifecycle/archive/consolidation; SkillOpt owns staged eval evidence and adoption recommendations. Native Hermes metadata is read-only advisory guard input."
ADOPT_PROPS = {"run_id": {"type": "string"}, "force": {"type": "boolean", "default": False}, "confirmation": {"type": "string", "description": "Typed confirmation; must exactly equal ADOPT <run_id> unless non_interactive_override=true."}, "non_interactive_override": {"type": "boolean", "default": False, "description": "Deliberate CI/test override; core gate checks still apply. Not cron-safe; no auto-adopt or curator replacement."}}
ROLLBACK_PROPS = {"run_id": {"type": "string"}, "force": {"type": "boolean", "default": False}, "confirmation": {"type": "string", "description": "Typed confirmation; must exactly equal ROLLBACK <run_id> unless non_interactive_override=true."}, "non_interactive_override": {"type": "boolean", "default": False, "description": "Deliberate CI/test override; core guard checks still apply. Not cron-safe; no auto-rollback or curator replacement."}}
FULL_PROPS = {
    **COMMON_HOME,
    "skill": {"type": "string"},
    "query": {"type": "string"},
...
SCHEMAS = {
    ...
    "hermes_skillopt_review": _schema("Review a staged SkillOpt run with eval_level/evidence_maturity, read-only native Hermes metadata, gate score, accepted/rejected status, paths, artifact refs, and optional diff/report preview. run_id may be omitted or 'latest'; summary returns decision-first slim output; digest is digest-only/manual for an already-staged run and returns Telegram-friendly path/hash refs only; it is not a scheduled default. " + BOUNDARY_NOTE, {**COMMON_HOME, "run_id": {"type": "string"}, "latest": {"type": "boolean", "default": False}, "summary": {"type": "boolean", "default": False}, "digest": {"type": "boolean", "default": False, "description": "Manual Telegram-friendly slim digest with separated readiness/adoptability fields and artifact refs, not raw report/diff; not a scheduled default."}, "include_diff_chars": {"type": "integer", "default": 4000}, "slim": {"type": "boolean", "default": False, "description": "When true, omit large diff/report previews and return path/hash artifact references only."}}),
    ...
    "hermes_skillopt_rollback": _schema("HIGH RISK writeback: rollback an adopted run in the active Hermes profile only, using a validated backup manifest and backup SKILL.md after current-sha guard unless force=true. Never automatic, never cron-safe, and not curator replacement. " + BOUNDARY_NOTE, ROLLBACK_PROPS, ["run_id"]),
