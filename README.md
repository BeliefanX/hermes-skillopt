...
Status/scout-style surfaces expose `hermes-skillopt-tool-safety-v1` metadata: read-only tools are low risk and cron-safe; report-only tools may write only explicit guarded reports; staged artifact creators are medium risk and not cron-safe; adopt/rollback/upstream-update are high risk and never cron-safe. Scheduled automation should be limited to scout, doctor, eval-pack-inventory, and review digest outputs over existing runs; never trigger optimize, adopt, rollback, cleanup, upstream update, or eval-pack write/promote actions.
