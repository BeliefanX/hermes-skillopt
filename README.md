...
- `hermes_skillopt_scout`: read-only notification-ready scout for skills, eval-pack readiness, recent staged runs, artifact hygiene, advisory metadata, and safe next commands. It never calls full-run/optimize/adopt/rollback/fetch and its cron recommendation is scout-only/no-auto-adopt.
...
- **Manual / read-only scouting:** run `python3 -m hermes_skillopt.cli scout --skill <skill>` or `doctor` before any optimization. Scout is notification-safe and may be scheduled externally, but only as `scout`; do not schedule optimize/adopt/rollback.
- **Scheduled scout:** an external cron/systemd/Hermes schedule may run `hermes-skillopt --home <profile> scout --output skillopt/reports/scout.json` and notify on the JSON/digest. The emitted `cron_recommendation` deliberately says `create_cron_job: false` and `auto_adopt_from_cron: false`; it is a recommendation surface, not a cron writer.
