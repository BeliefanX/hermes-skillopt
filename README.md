...
`hermes-skillopt` is a standalone Hermes plugin for safe, staged optimization of a Hermes profile `SKILL.md`. It is **not** a fork or full port of Microsoft SkillOpt, and it does **not** replace Hermes' native curator. Curator remains the authority for lifecycle, archive, consolidation, hub/bundled/pinned state, and native skill ownership. SkillOpt owns staged eval evidence, artifact review, and adoption recommendations only; it keeps Hermes core unchanged and keeps all candidate changes reviewable and rollbackable.
...
- **Frozen target executor:** evaluates the current skill and each candidate under the same frozen target config, but production adoption for `frozen-hermes` / `frozen_hermes_target_execution_v1` now requires real Hermes runtime proof. The current sandbox/fixed internal runners are isolated review-only evidence: they record fingerprints/transcripts and block task commands, but cannot satisfy production adoption. A disabled-by-default `live-readonly` interface remains report-only unless a future real Hermes runtime adapter supplies the required proof.
...
- **Native Hermes boundary:** SkillOpt reads Hermes-owned skill metadata sidecars best-effort (`skills/.usage.json`, `.curator_state`, `.hub_manifest.json`, `.bundled_manifest.json`, `.manifest.json`) for advisory UX, provenance, and adopt guards. It never writes those native sidecars and does not create a replacement sidecar database.
- **Safety shell:** full runs write only staged artifacts; live writes require explicit `adopt`; `rollback` restores from guarded backups. Active profile resolution first tries Hermes' official home helper, then falls back to `HERMES_HOME`, then `~/.hermes`. Adopt/rollback perform post-write readback verification before reporting success. Report/eval outputs share a safe path guard that rejects live/runtime-sensitive profile/plugin/source paths and symlink escapes.
...
- `hermes_skillopt_scout`: read-only notification-ready scout for skills, eval-pack readiness, recent staged runs, artifact hygiene, advisory metadata, and safe next commands. It never calls full-run/optimize/adopt/rollback/fetch, and CLI scout does not write a report unless `--output` is explicitly supplied. Its cron recommendation is scout-only/no-auto-adopt.
...
- `hermes_skillopt_eval_pack_doctor`: focused read-only eval-pack diagnostics and safe next actions; never writes, runs evals, adopts, or fetches. CLI `--digest` returns notification-friendly diagnostics.
- `hermes_skillopt_eval_pack_workflow`: read-only eval-pack workflow/authoring summary that combines inventory, doctor status, review-only provenance, safe next commands, and a promotion checklist. It does not write packs or imply production promotion.
- `hermes_skillopt_skill_readiness_queue`: read-only high-value skill readiness queue (also exposed as CLI aliases `readiness-queue` and `skill-queue`) that prioritizes skills by usage/package/eval readiness, surfaces native/curator guards, and emits safe next commands only.
- `hermes_skillopt_eval_pack_autopilot`: plan/read-only by default; an explicit `write_draft=true`/CLI `--write-draft` writes only a guarded generated review-only draft pack.
...
- `hermes_skillopt_compare_upstream_pin`: read-only lock/clone comparison; no fetch, merge, or write.
- `hermes_skillopt_benchmark_parity_status`: read-only status labeling Hermes-native benchmark coverage vs upstream parity levels; no rollout/adopt/writeback.
...
Status/scout-style surfaces expose `hermes-skillopt-tool-safety-v1` metadata: low-risk/read-only does **not** automatically mean cron-safe. The only cron-safe default tools are `scout`, `doctor`, `eval-pack-inventory`, and `eval-pack-doctor` (typically with `--digest`/digest output). `review --digest` is digest-only/manual over an already-staged run and is not a scheduled default. Report-only tools may write only explicit guarded reports; staged artifact creators are medium risk and not cron-safe; adopt/rollback/upstream-update are high risk and never cron-safe. Do not schedule status, review, optimize, full-run, adopt, rollback, cleanup, upstream update, or eval-pack write/promote/skeleton actions.
...
- `target_executor` / `target_backend`: `auto|replay|sandbox|scorecard|frozen-hermes|frozen_hermes_target_execution_v1|live-readonly`; controls the frozen evaluator, separate from the optimizer backend. `frozen-hermes` currently routes to the constrained sandbox/fixed internal runner and records review-only frozen-target evidence; it is not production-adoption evidence because it lacks real Hermes runtime invocation proof. `live-readonly` is disabled/report-only without future real-target evidence.
...
- `force`: only affects adopt/rollback current-sha guard behavior where exposed; it does not bypass artifact, profile, validation, production, test, complete frozen-runtime evidence, reviewer, provenance-hash, or native Hermes conflict guards.

CLI help confirms the supported surface:

```bash
python3 -m hermes_skillopt.cli --help
python3 -m hermes_skillopt.cli scout --help
...
python3 -m hermes_skillopt.cli eval-pack-workflow --help
python3 -m hermes_skillopt.cli skill-readiness-queue --help
python3 -m hermes_skillopt.cli skill-quality --help
...
- **Manual / read-only scouting:** run `python3 -m hermes_skillopt.cli scout --skill <skill>` or `doctor` before any optimization. Scout is notification-safe and, without `--output`, returns JSON without writing a report.
- **Scheduled read-only usage only:** external cron/systemd/Hermes schedules should be limited to `scout --digest`, `doctor --digest`, `eval-pack-inventory --digest`, and `eval-pack-doctor --digest`. `review --digest` is useful notification text for a human-selected existing run, but it is digest-only/manual and not a cron-safe default. If a file artifact is needed, pass an explicit guarded `--output` such as `hermes-skillopt --home <profile> scout --output skillopt/reports/scout.json`; omit `--output` for no write. The emitted `cron_recommendation` deliberately says `create_cron_job: false` and `auto_adopt_from_cron: false`; it is a recommendation surface, not a cron writer. Do not schedule status/review/optimize/full-run/adopt/rollback/upstream-update, eval-pack write/promote, skill-quality skeleton, or cleanup actions.
- **Review / authoring loop:** run `python3 -m hermes_skillopt.cli optimize --intent review --skill <skill> --eval-file <review-or-curated-pack>` and inspect with `python3 -m hermes_skillopt.cli review latest --summary`, `python3 -m hermes_skillopt.cli review latest --digest`, or `python3 -m hermes_skillopt.cli review latest --slim`. This is the default safe workflow for proposed skill edits.
- **Production candidate:** run `python3 -m hermes_skillopt.cli optimize --intent production --skill <skill> --eval-file <curated-production-pack> --optimizer-backend hermes --gate-mode strict --target-executor frozen-hermes` only after `doctor` and eval inventory show an explicit curated train/validation/test pack **and** the target path can provide the required real Hermes runtime evidence. On the current sandbox/fixed-runner implementation, production intent remains staged/review-only because frozen-target runtime evidence is incomplete. Live writeback requires `hermes-skillopt adopt <run_id> --confirm "ADOPT <run_id>"` and the core re-checks artifact, production, test, provenance, runtime-evidence, reviewer, current-SHA, native metadata fingerprint, and native conflict gates.
- **Gated staged optimize:** smoke/review/production optimize all remain staged. `production` requires explicit curated eval-file evidence, strict gates, non-mock optimizer provenance, and complete adoption-eligible target runtime evidence before it can become an adoption candidate.
...
Batch plans are data-only JSON objects using `schema_version: hermes-skillopt-batch-plan-v1`:

```json
{
  "schema_version": "hermes-skillopt-batch-plan-v1",
  "plan_id": "review-small-fleet",
  "defaults": {"iterations": 1, "candidate_count": 1, "gate_mode": "strict", "backend": "auto", "target_executor": "replay"},
  "budget": {"max_jobs": 3, "max_total_iterations": 3, "max_total_candidates": 3},
  "jobs": [
    {"skill": "my-skill", "eval_file": "skillopt/evals/my-skill.json", "query": "tighten safety wording"}
  ]
}
```
...
`manifest.json` records SHA-256 hashes for staged artifacts plus `skillopt-provenance-v2`: plugin repo/commit, upstream lock, eval/task fingerprint, optimizer_backend/target_backend configs, gate policy, profile/skill fingerprints, and production eval policy fingerprint. `review`, `adopt`, and `rollback` re-check artifact integrity before trusting the run. At adopt time, SkillOpt also reloads the verified `gate_results.json`, `test_results.json`, `val_items.jsonl`, `test_items.jsonl`, `candidate_summary.json`, `evidence.json`, and `proposed_SKILL.md` artifacts and independently re-derives production/test eligibility, production eval policy, and provenance fingerprint; manifest-only edits cannot make a review-only or non-production run adoptable. Candidate summaries/manifests also preserve a score ledger that separates `production_curated_score` from `review_only_score`, per-task score deltas, expected-term/assertion changes, and held-out test sensitivity warnings. Slim review, digest, scout, fleet, and hygiene surfaces include `score_provenance` with target executor/backend, optimizer backend, eval pack id/version/path/fingerprint, policy fingerprint, score source (`production_curated_eval_pack`, `review_only_or_non_production_eval`, or `mock_review_only`), split labels, score fields, fingerprints, and warnings. If a held-out test score is present without a `heldout_test_sensitivity` artifact/summary, the warning must be treated as a sensitivity caveat, not a production claim.

Native-boundary artifacts include `native_hermes_metadata` (`hermes-native-skill-metadata-snapshot-v1`) and `native_hermes_adopt_guard` (`hermes-native-adopt-guard-v1`) wherever a review/adopt surface needs them. These are read-only snapshots over Hermes-owned `.usage.json`/curator/hub/bundled/provenance sidecars. They include sidecar readability and SHA-256 fingerprints so reviewers can see whether native metadata changed since staging; SkillOpt does not write native sidecars. Evidence maturity is surfaced through `evidence_ledger` (`hermes-skillopt-evidence-ledger-v1`) with `eval_level`, `evidence_maturity`, `production_runtime_ready`, `complete_frozen_evidence`, real-runtime invocation/evidence booleans, task-command status, reviewer-gate status, and blockers. Static/replay/sandbox/live-disabled evidence remains review-only unless the ledger shows complete frozen Hermes target execution, real Hermes runtime invocation/evidence, no task command execution, a non-internal runner, and an adoptable deterministic reviewer gate.
...
python3 -m hermes_skillopt.cli eval-pack-inventory --digest
...
python3 -m hermes_skillopt.cli eval-pack-doctor --skill my-skill --digest
python3 -m hermes_skillopt.cli eval-pack-workflow --skill my-skill
python3 -m hermes_skillopt.cli skill-readiness-queue --skill my-skill
python3 -m hermes_skillopt.cli skill-quality --skill my-skill
python3 -m hermes_skillopt.cli skill-quality --skill my-skill --digest
...
`eval-pack-doctor` is the focused read-only diagnostic layer over inventory. It emits per-skill `safe_commands` for autopilot planning, explicit draft writing, and review promotion, and never writes, runs evals, adopts, or fetches. `eval-pack-workflow` is the read-only authoring overview: it combines inventory/doctor status, review-only provenance labels, safe next actions, and a production-promotion checklist without writing a pack. `skill-readiness-queue`/`skill-queue` ranks high-value skills by advisory usage/package/eval readiness, keeps native/pinned/hub/bundled/curator-managed guards visible, and emits only safe diagnostic/authoring commands. `skill-quality`/`skill-lint` is a read-only skill quality and eval-readiness report; `--create-eval-skeleton` is an explicit guarded review-only scaffold write and cannot edit `SKILL.md` or become production evidence. `eval-pack-autopilot` is also plan/read-only by default (`mode: eval_pack_autopilot_plan_read_only`) and is not a default cron surface; only `--write-draft` writes a guarded generated draft under the eval/report-safe path policy. Autopilot drafts, correction/context seed packs, session-mined packs, scaffolds, skill-quality skeletons, and negative/boundary packs are all review-only (`production_gate_eligible: false`) and are intended to help humans build regression coverage, not to create production evidence automatically. `eval-pack-promote` promotes such a draft to a curated review pack by default; production promotion requires `--production` plus explicit `--production-policy` and `--eval-execution-contract`, still never adopts and still never writes live skills. The WebUI one-click promotion intentionally exposes review promotion only; production promotion remains a CLI/tool action with explicit policy/contract files.
...
- `sandbox`: isolated fixed-internal-runner sandbox for review evidence; it is non-production for adoption because it does not invoke real Hermes runtime.
- `frozen-hermes` / `frozen_hermes_target_execution_v1`: alias for the current sandbox-based frozen Hermes target execution path. The contract requires target config/provider/model/toolset/session fingerprints, isolated runtime, permission policy with task commands disabled, transcript/trajectory artifact, execution-result scoring, and explicit real Hermes runtime invocation proof. Current sandbox/fixed internal runner evidence is review-only/non-production because `real_hermes_runtime_evidence` is false and the backend is an internal runner. It is not upstream benchmark execution/parity or arbitrary live agent command execution.
...
- `frozen_hermes_target_execution_v1`: evidence contract for adoption-eligible frozen Hermes target execution. It requires target config fingerprint, provider/model/toolset/session/runtime fingerprints, isolated runtime proof, declared permissions with task-provided commands disabled, transcript/trajectory artifact or fingerprint, scoring from execution output rather than only static `SKILL.md` text, and explicit real Hermes runtime invocation proof. Missing real-runtime proof or use of sandbox/replay/scorecard/fixed internal runners makes the evidence review-only and production-ineligible.

Sandbox/frozen-Hermes mode creates a temporary isolated HOME/HERMES_HOME/workspace, writes `SKILL.md` inside that sandbox, runs a fixed internal runner, captures transcript/exit/timeout, and does not write the live profile. This is useful review evidence only. Task-provided commands in `fixtures.command` or `metadata.command` are blocked with `SANDBOX_COMMAND_BLOCKED` and are not production-gate eligible. Do not document or rely on sandbox as adoption-eligible frozen Hermes evidence, an arbitrary shell executor, or a live agent command path.
...
- `conformance` runs local deterministic checks (`compileall` and pytest args) and returns a `hermes-skillopt-conformance-v1` JSON payload. It does not contact upstream, run external benchmark code, or require live Hermes services. It writes no default repo-root report (`skillopt_conformance_report.json` is not created); pass explicit `--output <report.json>` to write a file. Report output uses the same safe path guard and cannot overwrite plugin source or live profile/runtime files.
...
Target evidence summaries also treat `task_commands_executed=true`, missing `real_hermes_runtime_evidence`, missing `real_hermes_runtime_invocation`, or `non_production_internal_runner` as incomplete evidence: task-provided commands are blocked by default and fixed internal runner output cannot be used to make production/adoption claims.

These utilities are useful for local regression evidence. Upstream benchmark bridge status is **import-only / no-execution**: safe JSON manifest conversion is available, but true upstream benchmark execution remains unsupported. Sandbox/fixed-runner frozen-Hermes output is review-only local evidence and does not turn imported upstream-style manifests into Microsoft SkillOpt benchmark parity or production adoption proof. These utilities do not claim Microsoft benchmark parity, real cross-model transfer, or production performance improvements unless you supply and verify those evals yourself.
...
- staged artifact hashes verify, and adopt-time cross-checks re-derive gate/test eligibility, target runtime-evidence eligibility, production policy, provenance, candidate summary, and proposed skill SHA from those hashed artifacts
- `evidence_ledger.production_runtime_ready == true`: complete `frozen_hermes_target_execution_v1` evidence, real Hermes runtime invocation/evidence, no task-provided commands, not an internal review-only runner, and deterministic reviewer gate adoption status
- native Hermes adopt guard allows the target: hub-installed, bundled, pinned, archived, and curator-managed skills are diagnostic-only/blocked by default, and native metadata fingerprint must still match the staged snapshot
- target path resolves under the active profile `skills/`
- current live skill SHA matches the staged original SHA unless an explicit force path is used; force does not bypass native conflict guard, artifact/provenance/evidence gates, reviewer gate, proposed-SHA checks, or production/test gates
- staged proposed skill SHA matches the manifest

Adopt creates `$HERMES_HOME/skillopt/backups/<timestamp-run-id>/` containing the previous `SKILL.md` and backup manifest, writes the proposed skill, then reads the runtime-visible `SKILL.md` back and verifies SHA/frontmatter before returning success. `rollback(run_id)` restores only from that verified backup and checks run id, target path, relative path, original/adopted/proposed SHA, current live SHA unless forced, and post-restore readback.
...
The optional WebUI is a packaged React/shadcn-style frontend served by FastAPI/Uvicorn:
...
When served on a local/LAN origin, the WebUI advertises PWA/mobile standalone metadata (`manifest.webmanifest`, icons, viewport safe-area hints, Apple mobile tags, and a service worker) so browsers can offer “Add to Home Screen”. The service worker cache boundary is intentionally narrow: it only caches static PWA assets/icons plus the generic offline fallback, and treats `/`, API/run/file/result/status-style routes as network-only/no-store so staged run data, profile paths, artifacts, and operation results are not replayed from cache. The offline fallback is static safety copy only and contains no run/profile/status data.
...
Updating upstream refreshes clone/lock metadata only. It does not merge upstream code, change plugin behavior, or adopt any skill. Upstream benchmark adapter levels are intentionally conservative: `json_import_only` and `pinned_manifest_replay` are supported data-conversion evidence levels; `pinned_upstream_execution` and `parity_evidence_complete` are future/unsupported. Do not report `benchmark`, `eval-only`, sandbox/fixed-runner frozen-Hermes evidence, or imported manifests as full Microsoft SkillOpt benchmark parity today.
...
Recent P0/P1/P2/P3/P4/P5 hardening verification also includes targeted tests for safe report output paths, protected bounded-edit regions/headings, frozen-Hermes evidence downgrades for non-real-runtime runners, production-vs-review score ledgers, batch preflight/run budgets, eval-pack inventory/scaffold gaps, fleet read-only planning, transfer/import/conformance safety, WebUI staged-only wizard/review-console APIs, artifact hygiene reporting, decision-first summaries, staged-only slow/rejected history, and held-out sensitivity warnings.
