...
def main() -> int:
    ...
    conf = sub.add_parser("conformance", help="Run local conformance and return JSON. Default mode=quick is a smoke suite, not full repo health; use --mode full for all pytest tests.")
    conf.add_argument("--output", help="Optional guarded .json report path; omitted means no file is written"); conf.add_argument("--pytest-arg", action="append", dest="pytest_args"); conf.add_argument("--timeout", type=int, default=180); conf.add_argument("--mode", choices=["quick", "full"], default="quick")
